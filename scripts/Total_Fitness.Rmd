...
JM: is Poisson appropriate?  assumes mean = variance...
...
rep_output_fits_wl2 %>%
  ...
  unnest(tidyboot) %>%
  ggplot(aes(x=pop)) +
  geom_col(aes(y=meanTotalFitness), data = wl2_fitness_means, fill="skyblue") +
  geom_pointrange(aes(y=statistic, ymax = conf.high, ymin = conf.low, color=name), position = position_dodge(width = .75)) +
...
rep_output_fits_ucd %>%
  ...
  unnest(tidyboot) %>%
  ggplot(aes(x=pop)) +
  geom_col(aes(y=meanTotalFitness), data = ucd_fitness_means, fill="skyblue") +
  geom_pointrange(aes(y=statistic, ymax = conf.high, ymin = conf.low, color=name), position = position_dodge(width = .75)) +
   ylab("Reproductive Output") +
  scale_color_viridis_d() +
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=.5))
