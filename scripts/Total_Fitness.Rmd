...
ucd_gowers <- read_csv("../output/Climate/Gowers_UCD.csv") %>% 
  ...
  mutate(UCD_Lat=38.53250, UCD_Long=-121.7830) %>% 
  mutate(Geographic_Dist=distHaversine(cbind(UCD_Long, UCD_Lat), cbind(Long, Lat))) %>% # Calculate the distance using the haversine formula (dist in meters)
...
wl2_gowers_2023 <- read_csv("../output/Climate/Gowers_WL2.csv") %>% 
  ...
  mutate(Geographic_Dist=distHaversine(cbind(WL2_Long, WL2_Lat), cbind(Long, Lat))) %>% # Calculate the distance using the haversine formula
...
cor.norm = cor(dist_normalized_ucd) #test correlations among the traits
corrplot(cor.norm) #geo dist not strongly correlated to any of the climate distances, yay!
```
...
ucd_establishment <- ucd_surv %>% 
  ...
  select(block:rep, elevation.group:Wtr_Year_GD_Historical, Geographic_Dist, death.date) %>% 
...
GSCD <- ucd_establishment %>% 
  group_by(pop, elev_m, GrwSsn_GD_Recent, Wtr_Year_GD_Recent) %>% 
  summarise(meanEst=mean(Establishment, na.rm = TRUE), semEst=sem(Establishment, na.rm=TRUE)) %>% 
  ggplot(aes(x=GrwSsn_GD_Recent, y=meanEst, color=GrwSsn_GD_Recent, group = pop)) +
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanEst-semEst,ymax=meanEst+semEst),width=.02, linewidth = 2) +
  #geom_text_repel(aes(x = GrwSsn_GD_Recent, y = meanEst,
  #          label = `pop`),
...
WYCD <- ucd_establishment %>% 
  group_by(pop, elev_m, GrwSsn_GD_Recent, Wtr_Year_GD_Recent) %>% 
  summarise(meanEst=mean(Establishment, na.rm = TRUE), semEst=sem(Establishment, na.rm=TRUE)) %>% 
  ggplot(aes(x=Wtr_Year_GD_Recent, y=meanEst, color=Wtr_Year_GD_Recent, group = pop)) +
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanEst-semEst,ymax=meanEst+semEst),width=.02, linewidth = 2) +
  #geom_text_repel(aes(x = Wtr_Year_GD_Recent, y = meanEst,
  #          label = `pop`),
  #      min.segment.length = 0.8,
  #      max.overlaps = 100,
  #      #label.padding = 1,
...
GD <- ucd_establishment %>% 
  ...
  summarise(meanEst=mean(Establishment, na.rm = TRUE), semEst=sem(Establishment, na.rm=TRUE)) %>% 
  ggplot(aes(x=Geographic_Dist, y=meanEst, color=Geographic_Dist, group = pop)) +
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanEst-semEst,ymax=meanEst+semEst),width=.02, linewidth = 2) +
  ...
  theme_classic() + 
  scale_y_continuous(expand = c(0.01, 0)) +
  labs(y="Establishment", x="Geographic Distance (m)", color="Geographic Distance") +
  scale_color_viridis(option="mako", direction = -1) +
  theme(text=element_text(size=25), axis.text.x = element_text(angle = 45,  hjust = 1))

ucd_establishment_FIG <- ggarrange(GSCD, WYCD, GD, ncol=2, nrow=2) 
...
WL2_establishment_FIG <- ggarrange(GSCD, WYCD, GD, ncol=2, nrow=2) 
...
ucd_surv_to_rep <- ucd_surv %>% 
  ...
  select(block:rep, elevation.group:Wtr_Year_GD_Historical, Geographic_Dist, bud.date, death.date, SurvtoRep_Y1) 
...
winter_surv <- post_winter_clean %>% 
  ...
  select(block:rep, elevation.group:Wtr_Year_GD_Historical, Geographic_Dist, death.date, WinterSurv) 
...
wl2_surv_to_rep_y2 <- wl2_surv_y2 %>% 
  ...
  select(Pop.Type:Genotype, elevation.group:Wtr_Year_GD_Historical, Geographic_Dist, bud.date, death.date, SurvtoRep_y2) 
...
ucd_total_fitness <- left_join(ucd_establishment, ucd_surv_to_rep) %>% 
  ...
  select(block:Wtr_Year_GD_Historical, Geographic_Dist, Establishment, SurvtoRep_Y1:fruits) %>% 
...
wl2_total_fitness <- left_join(wl2_establishment, wl2_surv_to_rep_y1) %>% 
  ...
  select(block:Wtr_Year_GD_Historical, Geographic_Dist, Establishment, SurvtoRep_Y1:y2_fruits) %>% 
...
GD <- ucd_total_fitness %>% 
  ...
  theme_classic() + 
  scale_y_continuous(expand = c(0.01, 0)) +
  labs(y="Total Fitness", x="Geographic Distance (m)", color="Geographic Distance") +
  scale_color_viridis(option="mako", direction = -1) +
  theme(text=element_text(size=25), axis.text.x = element_text(angle = 45,  hjust = 1))

ucd_total_fitness_FIG <- ggarrange(GSCD, WYCD, GD, ncol=2, nrow=2) 
...
WL2_total_fitness_FIG <- ggarrange(GSCD, WYCD, GD, ncol=2, nrow=2) 
ggsave("../output/WL2_Traits/WL2_Total_Fitness_SCATTERS.png", width = 24, height = 18, units = "in")
```

### Stats
```{r}
wl2_total_fitness_scaled <- wl2_total_fitness %>% mutate_at(c("GrwSsn_GD_Recent","Wtr_Year_GD_Recent","Geographic_Dist"), scale)
wl2_total_fitness_gs_mod1 <- lmer(Total_Fitness ~ GrwSsn_GD_Recent + Geographic_Dist + (1|pop) + (1|block), data=wl2_total_fitness_scaled)
...
wl2_total_fitness_wy_mod1 <- lmer(Total_Fitness ~ Wtr_Year_GD_Recent + Geographic_Dist + (1|pop) + (1|block), data=wl2_total_fitness_scaled)
...
ucd_total_fitness_scaled <- ucd_total_fitness %>% mutate_at(c("GrwSsn_GD_Recent","Wtr_Year_GD_Recent","Geographic_Dist"), scale)
ucd_total_fitness_gs_mod1 <- lmer(Total_Fitness ~ GrwSsn_GD_Recent + Geographic_Dist + (1|pop) + (1|block), data=ucd_total_fitness_scaled)
...
ucd_total_fitness_wy_mod1 <- lmer(Total_Fitness ~ Wtr_Year_GD_Recent + Geographic_Dist + (1|pop) + (1|block), data=ucd_total_fitness_scaled)
