...
prob_fitness_GD_fits %>% mutate(glance=map(fit, glance)) %>% unnest(glance) %>% arrange(AIC) %>% select(-wflow:-sigma)
#water year models preferred by AIC 

prob_fitness_GD_fits %>% mutate(tidy=map(fit, tidy)) %>% unnest(tidy) %>%
  filter(str_detect(term, "GD") | term=="Geographic_Dist") %>%
  drop_na(p.value) %>%
  select(-wflow:-group)# %>%
#  arrange(p.value)
# recent and historical CD are sig, geo dist is not sig in those models. I think the GS_RECENT model had convergence issues. - need to check the map warnings 
```
