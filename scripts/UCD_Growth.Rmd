...
Merge
```{r}
all_ucd_size_loc <- left_join(all_ucd_size_all, gowersdist_all)
...
## Quick figure
```{r}
all_ucd_size_loc$parent.pop = factor(all_ucd_size_loc$parent.pop, levels=c('CC','TM2','SC','IH','BH','WV','FR','DPR','WR','WL1','SQ1','SQ2','WL2','YO4','CP2','CP3','LV3','SQ3','YO7','YO8','LV1','LVTR1','YO11'))
...
#check weird cases
all_ucd_size_loc %>% filter(parent.pop=="WV") %>% arrange(Genotype) #no issues 
all_ucd_size_loc %>% filter(parent.pop=="CP3") %>% arrange(Genotype) 
all_ucd_size_loc %>% filter(parent.pop=="SQ3") %>% filter(Genotype=="SQ3_4_3") #SOMETHING WEIRD HAPPENED WITH THIS PLANT 

all_ucd_size_loc %>% filter(parent.pop=="LV3") %>% filter(Genotype=="LV3_6_3")
```
