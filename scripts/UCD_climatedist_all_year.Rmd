...
gowers_calc <- function(data, indices=1:23, P) { #function with all of the code necessary for calculating gowers distance 
  #data = home_climates_all_ (flint and bioclim for home climates) ; P (14 if includes CWD)
  #need to make davis_range_prep before running this function 
  
  data <- data %>% filter(parent.pop==pops[indices]) # subset per bootstrap indices
 ...
 gowers_calc_per_pop_recent #%>% pull(Recent_Gowers_Dist) #make the result a vector 
   }

gowers_calc(recent_clim_boot, P=14) #the function works
gowers_calc(home_climates_all_recent, P=14) #results with 30-year avgs as input 

recent_clim_boot %>% filter(parent.pop==pops[1])
gowers.boot <- boot(data=recent_clim_boot, statistic=gowers_calc, R=1000, P=14) 
