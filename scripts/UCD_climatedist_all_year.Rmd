...
recent_clim_boot %>% filter(parent.pop=="YO8")
recent_clim_boot %>% filter(parent.pop==pops[23])

recent_clim_boot_nest <- recent_clim_boot %>% nest(.by=year)
...
gowers_calc <- function(data, indices, P) { #function with all of the code necessary for calculating gowers distance 
  ...
  range_merge <- bind_rows(data, davis_range_prep)
  ...
  davis_home_climate <- bind_cols(davis_climate_all, data) #add davis climate data to home climate data 
  ...
  dplyr::select(parent.pop, elevation.group, elev_m, year, ends_with("_gowers"))
 ...
 gowers_calc_per_pop_recent <- gowers_calc_each_var %>% #final gowers calc 
  ...
  dplyr::select(parent.pop, elevation.group, elev_m, year, Recent_Gowers_Dist)
...
#gowers_calc(home_climates_all_recent, P=14) #results with 30-year avgs as input 

#recent_clim_boot %>% filter(parent.pop==pops[1])

gowers.boot <- boot(data=recent_clim_boot_nest, statistic=gowers_calc, R=1000, P=14) #will sample each row (year) with replacement 
gowers.boot$t0
str(gowers.boot)
#mean(gowers.boot$t[1,])
plot(gowers.boot, index = 1)
plot(gowers.boot, index = 2)
plot(gowers.boot, index = 3)
plot(gowers.boot, index = 9)

boot.ci(gowers.boot, type="norm", index = 1) # for the first pop
tidy(gowers.boot,conf.int=TRUE,conf.method="norm") #all pops 
```

```{r}
pops <- recent_clim_boot %>% select(parent.pop) %>% unique()

boot_gowers_avgs <- as_tibble(gowers.boot$t0) %>% rename(Recent_Gowers_Dist = value)

boot_gowers_avgs_pops <- bind_cols(pops, boot_gowers_avgs) %>% arrange(Recent_Gowers_Dist)

boot_gowers_avgs_pops %>% ggplot(aes(fct_reorder(parent.pop, Recent_Gowers_Dist), y=Recent_Gowers_Dist)) +
  geom_col()
