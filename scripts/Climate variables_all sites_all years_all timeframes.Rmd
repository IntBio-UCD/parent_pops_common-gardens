...
flint_all_year_wtr_yr <- flint_all_year_mosnums %>% 
  ...
  filter(if_else(parent.pop!="WL2_Garden", wtr_yr<2024, wtr_yr<=2024)) %>% #only keep the WL2 garden climate for 2024 water year #JRG, why not keep it in until we need to use it?  that way, if we wanted to look at the range of variation (or whatever), we have it.
...
IH_sept_last_month <- nosnow_60first_last_month %>% 
  ...
  arrange(grow_yr) #14 years left for IH with no last month (last month = next year's Sept) #JRG: I thought we were going to use the max of the full time series when there wasn't any last month?  
...
WL2_2024_first_month <- WL2_2024_clim %>%
  group_by(parent.pop, elevation.group, elev_m, year_cal) %>% #JRG: why group if you've already filtered to the one site and year?
  filter(pck==0) %>% 
  filter(tmn > 0) %>% 
  arrange(month) %>% 
  filter(row_number()==1) #get first month for each pop and year with no snowpack for germ
