...
snow_60_last_month_col %>% filter(is.na(lastmonth)) #226 cases where there isn't a lastmonth #*no snow_pack, or min temp > -5 
#***WHAT TO DO FOR THESE CASES? Make the last month 12 (Dec) OR check the next calendar year to see when pck > 70

snow_60_last_month_col %>% filter(lastmonth==firstmonth+1) %>% arrange(parent.pop, year_cal) 
...
snow_60_last_month_col %>% filter(lastmonth==firstmonth+2) #8 cases where there was sig snowpack in the second month after the first month of 0 snowpack 
heavysnow_60 <- snow_60_last_month_col %>% filter(lastmonth<=firstmonth+2) %>% filter(month==firstmonth)
summary(heavysnow_60) #*min temps range from just above freezing (0.080) to 3.31 (which seems like an acceptable temp given we vernalize at 4C)
unique(heavysnow_60$parent.pop) #9 pops included
#START THE CHECK IN FEB FOR POPS > 6000 FT? 
heavysnow_60 %>% filter(elev_m>1800) #(ONLY 5 of the 9 ABOVE 6000 FEET)
xtabs(~firstmonth, data=heavysnow_60) #most of these have a first month of 1 or 2, but a few = 8 or 9
xtabs(~lastmonth, data=heavysnow_60) #most of these have a last month of 2 or 3, but a few have 10 or 11
heavysnow_60 %>% filter(firstmonth>2)
snow_60_last_month_col %>% filter(parent.pop=="LV1", year_cal==1982) # a few cases with high snowpack all year except Aug-Nov, these cases are fine to leave 

#check what the average start and end months are for these pops
#check what the min start months are for these pops
no_heavy_snow <- snow_60_last_month_col %>% filter(lastmonth > firstmonth+2)
summary(no_heavy_snow) #earliest last month = 10
no_heavy_snow %>% 
  group_by(parent.pop, elevation.group) %>% 
  summarise(minStart=min(firstmonth), meanStart=mean(firstmonth), 
            maxEnd=max(lastmonth), meanEnd=mean(lastmonth)) %>% 
  arrange(minStart) #mid elevation populations have some years where they could grow the entire year 
...
snow_60_last_grw_month_col %>% filter(is.na(lastmonth), !is.na(lastmonth_grwyr)) #use the grow year last month for th
