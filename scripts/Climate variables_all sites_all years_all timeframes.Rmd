...
nosnow_60_last_month_col %>% filter(firstmonth>4) #1 case in BH (2014) where first month is after December, this is fine, right? #JRG: yes, probably real - that was during the 2012-2015 drought
...
IH_sept_last_month <- nosnow_60first_last_month %>% 
  ...
  arrange(grow_yr) #14 years for IH where the last month = next year's Sept which is technically month 13 due to the way "grow years" are set up. I confirmed this by checking for CWD > 85.15 in the "nosnow_60_last_month_col %>% filter(parent.pop=="IH")" output in the above code chunk
...
#***WHAT TO DO FOR THESE CASES? check the next calendar year to see when pck > 70 and make the last month 12 (Dec) for the ones without that #JRG - let's use the max  of last month for these cases.  
...
WL2_2024_first_month <- WL2_2024_clim %>%
  group_by(parent.pop, elevation.group, elev_m,  Lat, Long, year_cal) %>% #need to group here so that "row_number()==1" will get the first month for each pop, not just the first pop in the data frame 
  filter(pck==0) %>% 
  filter(tmn > 0) %>% 
  arrange(month) %>% 
  filter(row_number()==1) #get first month for each pop and year with no snowpack for germ
...
bio8_60 <- wettest_month %>% #bio8 = tmean_wettest_month
  dplyr::select(parent.pop, elevation.group, elev_m,  Lat, Long, year, tmean_wettest_month=tavg)
bio9_60 <- driest_month %>% #bio9 = tmean_driest_month
  group_by(parent.pop, elevation.group, elev_m,  Lat, Long, year) %>% 
  summarise(tmean_driest_month=mean(tavg)) #taking the average b/c some years where there are multiple driest months 
bio18_60 <- warmest_month %>% #bio18 = ppt_warmest_month
  dplyr::select(parent.pop, elevation.group, elev_m,  Lat, Long, year, ppt_warmest_month=ppt)
bio19_60 <- coldest_month %>% #bio19 = ppt_coldest_month
  dplyr::select(parent.pop, elevation.group, elev_m,  Lat, Long, year, ppt_coldest_month=ppt)
# Merge:
