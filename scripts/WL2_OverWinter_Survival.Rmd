...
Note: Need to make a climate distance of 2024 garden climate - recent and historical home climates (up to 2023)
...
```{r}
wl2_wtr_year_sub_recent_2023 <- read_csv("../output/Climate/full_year_Subtraction_Dist_from_Home_WL2_Recent.csv") %>% 
  select(parent.pop, Wtr_Year_TempDist_Recent=ann_tmean_dist, Wtr_Year_PPTDist_Recent=ann_ppt_dist)
wl2_wtr_year_sub_historic_2023 <- read_csv("../output/Climate/full_year_Subtraction_Dist_from_Home_WL2_Historical.csv") %>% 
  select(parent.pop, Wtr_Year_TempDist_Historic=ann_tmean_dist, Wtr_Year_PPTDist_Historic=ann_ppt_dist)

wl2_sub_dist_2023 <- wl2_wtr_year_sub_recent_2023 %>% 
  left_join(wl2_wtr_year_sub_historic_2023) %>% 
  rename(pop=parent.pop) %>% 
...
### Wtih 2023 Gowers
```{r}
winter_surv_23 <- post_winter_clean %>% 
  left_join(wl2_gowers_2023) %>% 
  filter(death.date == "A" | death.date == "B" | death.date == "C" | death.date == "D") %>% 
  mutate(WinterSurv=if_else(death.date=="D", 0, 1)) %>% 
  select(block:rep, elevation.group:Wtr_Year_GD_Historical, Geographic_Dist, Elev_Dist, death.date, WinterSurv) 
...
```{r}
winter_surv_sub_dist_23 <- winter_surv %>% 
  select(block:rep, WinterSurv) %>% 
...
WYCD_recent <- winter_surv_23 %>%
  ...
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanSurv-semSurv,ymax=meanSurv+semSurv),width=.02,linewidth = 2) +
  annotate("text", x = 0.4, y= 0.24, label = "WL2", 
           colour = "purple", fontface="bold", size = 22 / .pt) +
  theme_classic() + 
  scale_y_continuous(expand = c(0.01, 0)) +
  labs(y="Winter Survival", x="Recent Water Year CD", colour="Elevation (m)") +
  theme(text=element_text(size=25))
ggsave("../output/WL2_Traits/WL2_WinterSurv_2023ClimDist.png", width = 10, height = 5, units = "in")
```
...
wl2_winter_surv_scaled_23 <- winter_surv_23 %>% 
  filter(pop!="WR") %>% 
  mutate_at(c("Wtr_Year_GD_Recent","Wtr_Year_GD_Historical", 
              "Geographic_Dist"), scale) 
...
winter_surv_sub_dist_scaled_23 <- winter_surv_sub_dist_23 %>% 
  filter(pop!="WR") %>% 
  mutate_at(c("Wtr_Year_TempDist_Recent",  "Wtr_Year_PPTDist_Recent", 
                 "Wtr_Year_TempDist_Historic", "Wtr_Year_PPTDist_Historic",
              "Geographic_Dist"), scale) 
...
winter_surv_tp_sub_dist_scaled_23 <- winter_surv_sub_dist_23 %>% 
  filter(pop!="WR") %>% 
  mutate_at(c("Wtr_Year_TempPpt_Recent", "Wtr_Year_TempPpt_Historic",
              "Geographic_Dist"), scale) 
```
...
surv_fits_wl2_23 <- surv_fits %>%
  mutate(fit = map(wflow, fit, data = wl2_winter_surv_scaled_23))
#mod_test <- glmer(WinterSurv ~ (1|pop/mf) + (1|block), data=wl2_winter_surv_scaled_23, family=binomial)
#summary(mod_test)
surv_fits_wl2_23 %>% mutate(glance=map(fit, glance)) %>% unnest(glance) %>% arrange(AIC) %>% select(-wflow:-sigma)
#model with just pop best by AIC and BIC, but nothing wrong with the full model, so use that 
```
