...
-   Get base models working

    -   Truncate analysis to only grow months (model likely cannot handle shrinkage)

-   Add weekly temperature (not enough soil moisture data)
...
#That would be pretty conservative so let's set 5 as a minimum sample size 
```
...
prior1_WL2 <- c(set_prior("normal(4,3)", nlpar="Hmin"), #(mean, sd)
            ...
            set_prior("gamma(1,2)", nlpar = "k"), #gamma constrains to be > 0 
            set_prior("gamma(4,2)", nlpar = "delta")
)
...
```{r}
fit1_WL2 <- brm(formula=f1_WL2, data=all_wl2_size_for_models, prior=prior1_WL2, cores = 4,  iter = 4000)

#Warning: There were 2722 divergent transitions after warmup.
...
```{r}
summary(fit1_WL2)
#Rhat and ESS are terrible  

pairs(fit1_WL2, regex = TRUE) #doesn't look great, these should be more cloud like than show patterns, I think...
```
...
#posterior distributions look bad
```
...
```{r}
random_fit2_WL2 <- brm(formula=random_f2_WL2, data=all_wl2_size_for_models, prior=prior1_WL2, cores = 4, iter = 3000) #increase iterations b/c more complex (helps with Rhat and ESS)
#Warning: There were 2482 divergent transitions after warmup. 
#Warning: There were 1 chains where the estimated Bayesian Fraction of Missing Information was low. Examine the pairs() plot to diagnose sampling problems
#Warning: The largest R-hat is 1.95, indicating chains have not mixed. Running the chains for more iterations may help.
#Bulk Effective Samples Size (ESS) is too low, indicating posterior means and medians may be unreliable.
#Tail Effective Samples Size (ESS) is too low, indicating posterior variances and tail quantiles may be unreliable.
```
