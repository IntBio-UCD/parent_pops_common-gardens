...
WYCD_temp_recent <- wl2_y1_surv_sub_dist %>% 
  ...
  geom_point(size=6) + 
  geom_errorbar(aes(ymin=meanEst-semEst,ymax=meanEst+semEst),width=.3,linewidth = 2) +
  geom_text_repel(aes(x = Wtr_Year_TempDist_Recent, y = meanEst,
          label = `pop`),
      min.segment.length = 1,
      max.overlaps = 100,
      #label.padding = 1,
...
wl2_y1surv_FIG_recent <- ggarrange(WYCD_recent, WYCD_temp_recent,
                                ...
                                font.label = list(size=30, 
                                                  face = "plain"), 
                                ncol=2, nrow=1) 
ggsave("../output/WL2_Traits/WL2_Y1_Surv_ClimTempDist_Recent_Text.png", width = 24, height = 9, units = "in")
```
...
wl2_y1_surv_sub_dist %>% 
  ...
  labs(y="Y1 Survival", x="Recent Growth Season PPT Dist") +
  theme(text=element_text(size=30))
#ggsave("../output/WL2_Traits/WL2_Y1_Surv_PPTSubDist_GRWSSN_Recent.png", width = 8, height = 6, units = "in")
```
