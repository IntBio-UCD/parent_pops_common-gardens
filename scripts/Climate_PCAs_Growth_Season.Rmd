...
lmer_results_grwssn %>% select(-data, -glm) %>% unnest(anova) %>%
  filter(PC=="PC1"| PC=="PC3") #the two other PCs for the manuscript 

#mod_test <- glm(PC1 ~ TimePd*elev_m*Lat, data=all_bioclim_flint_avgs_locs.pc)
#summary(mod_test)
#anova(mod_test)
```
...
home_sites_pca <- all_bioclim_flint_avgs_locs.pc_avg %>% 
  ...
  geom_vline(xintercept = 0, linetype="dashed") + geom_hline(yintercept = 0, linetype="dashed") +
  annotate("text", x = -0.06, y= -0.09, label = "WL2", colour = "purple", size=7) +
  geom_path(aes(group=group),arrow = arrow(length=unit(5, "points")), linewidth = .8) +
  annotate("text", x = -4.2, y = -4.6, label = "Low Temp \n Seasonality", size=6) +
  annotate("text", x = 3.6, y = -4.6, label = "High Temp \n Seasonality", size=6) +
  annotate("text", x = -4.7, y = -3, label = "Low Diurnal \n Range", size=6) +
  annotate("text", x = -4.7, y = 2.8, label = "High Diurnal \n Range", size=6) +
...
home_sites_pca + 
  ...
  annotate("text", x = -.6, y= -2.6, label = "WL2 Garden \n 2023", colour = "purple", size=6) +
  annotate("text", x = 2.7, y= -3, label = "WL2 Garden \n 2024", colour = "purple", size=6) 
