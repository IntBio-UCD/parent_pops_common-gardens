...
lmer_results_all_year %>% select(-data, -glm) %>% unnest(anova) %>% 
  filter(PC=="PC2"| PC=="PC3") #the two other PCs for the manuscript 
...
home_sites_pca <- all_bioclim_flint_avgs_locs.pc_avg %>%  
  ...
  geom_vline(xintercept = 0, linetype="dashed") + geom_hline(yintercept = 0, linetype="dashed") +
  annotate("text", x = 1.3, y= 0.52, label = "WL2", colour = "purple", size=7) +
  geom_path(aes(group=group),arrow = arrow(length=unit(5, "points")), linewidth = .8) +
  annotate("text", x = -4, y = -5.1, label = "Warm \n No Snow", size=6) +
  annotate("text", x = 3.5, y = -5.1, label = "Cold \n Snow", size=6) +
  annotate("text", x = -4.9, y = -3.3, label = "High Temp \n Seasonality", size=6) +
  annotate("text", x = -4.9, y = 1.3, label = "Low Temp \n Seasonality", size=6) +
...
home_sites_pca + 
  geom_point(data=filter(home_sites_pca$data, parent.pop == "WL2_Garden"), size=6, shape = 18, show_guide = FALSE) +
  annotate("text", x = 2.55, y= -3.5, label = "WL2 Garden \n 2023", colour = "purple", size=6) +
  annotate("text", x = -0.8, y= -3.3, label = "WL2 Garden \n 2024", colour = "purple", size=6) 
